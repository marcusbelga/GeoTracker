"""Deutsche Welle English — Middle East section scraper."""
...
class DWScraper(BaseScraper):
    ...
    async def fetch_articles(self) -> list[RawArticle]:
        ...
        urls = [
            "https://www.dw.com/en/middle-east/s-11827",
            "https://www.dw.com/en/iran/s-4226",
            "https://www.dw.com/en/israel/s-6218",
        ]
                ...
                for url in urls:
                    try:
                        ...
                        items = await page.query_selector_all(
                            "article, .news-item, [class*='column'], [class*='article']"
                        )

                        for el in items[:20]:
                            try:
                                link = await el.query_selector("a[href*='/en/']")
                                if not link:
                                ...
                                if not href:
                                    continue
                                full_url = (
                                    f"https://www.dw.com{href}"
                                    if href.startswith("/") else href
                                )
                                if "dw.com" not in full_url:
                                    continue
                                ...
                                desc = await el.query_selector("p, [class*='teaser']")
